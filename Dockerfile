...
RUN apk update && apk add make gcc g++ dbus-glib dbus-glib-dev openssl openssl-dev avahi db dbus curl db-dev bzip2 avahi-dev dbus-dev file bash dbus avahi acl libacl acl-dev py-dbus && curl -L "http://prdownloads.sourceforge.net/netatalk/netatalk-3.1.10.tar.bz2?download" | bunzip2 -c - | tar -x -f - -C /tmp && cd /tmp/netatalk-3.1.10 && ./configure --prefix=/netatalk  --with-acls --enable-zeroconf --enable-dbus  && make && make install && apk del make gcc g++ openssl-dev db-dev avahi-dev dbus-dev acl-dev dbus-glib-dev && rm -rf /tmp/netatalk-3.1.10 && cp /netatalk/etc/dbus-1/system.d/* /usr/share/dbus-1/system.d/
#VOLUME [ "/netatalk/etc" ]
COPY netatalk.sh /
