...
RUN apk update && apk add make gcc g++ dbus-glib dbus-glib-dev openssl openssl-dev avahi db dbus curl db-dev bzip2 avahi-dev dbus-dev file bash dbus avahi acl libacl acl-dev && curl -L "http://prdownloads.sourceforge.net/netatalk/netatalk-3.1.8.tar.bz2?download" | bunzip2 -c - | tar -x -f - -C /tmp && cd /tmp/netatalk-3.1.8 && ./configure --prefix=/netatalk  --with-acls --enable-zeroconf --enable-dbus  && echo '#define O_IGNORE 0' >> config.h && make && make install && apk del make gcc g++ openssl-dev db-dev avahi-dev dbus-dev acl-dev && rm -rf /tmp/netatalk-3.1.8
COPY netatalk.sh /
